...
export function makeDate(
  date_or_time_objct_or_string,
  format = "YYYY-MM-DD",
  { hour = undefined, minute = undefined, second = undefined, all = false, log=false, msg='', message='' } = {}
) {
  ...
  try {
    ...
    if(log) console.log('=======================< 111-222 makeDate() >=======================', message, {
      dateTime,
      typeof_dateTime: typeof dateTime,
      is_moment_instant: dateTime instanceof moment,
      'moment().isMoment()': moment.isMoment(dateTime),
      'is_momemnt_instance || isMoment': (dateTime instanceof moment) || moment.isMoment(dateTime),
      'moment().isValud()': moment().isValid(dateTime),
      is_date_instant: dateTime instanceof Date,
    });
    
     
    let date;  
    if (dateTime instanceof moment || moment.isMoment(dateTime)) {
      if(log) console.log('========== instance of moment =====');
      date = moment(dateTime); // as momemt object
      hour = hour ?? Number(date.format("H"));
      minute = minute ?? Number(date.format("m"));
      second = second ?? Number(date.format("s"));
    }
    else if (dateTime instanceof Date) {
      // If it's already a Date object, use it directly
      if(log) console.log('========== instance of Date =====');
      date = moment(dateTime);
      hour = hour ?? dateTime.getHours();
      minute = minute ?? dateTime.getMinutes();
      second = second ?? dateTime.getSeconds();
    }  
    else if (typeof dateTime === "string") {
      if(log) console.log('========== normal string =====');
      if (/^\d{1,2}:\d{1,2}\s?(AM|PM)?/i.test(dateTime)) { 
        ...
        try {
          if(log) console.log('========== in child try =====');
          date = moment(new Date(dateTime));
          hour = date.hours();
          minute = date.minutes();
          second = 0;
        } catch (error) {
          if(log) console.log('========== in child catch() =====');          
        }
      }
    } else {
      if(log) console.log('========== in last else >> output "" =====');          
      return "";
    }
    ...
    console.error("makeDate():error", (error, dateTime));
  }
}
