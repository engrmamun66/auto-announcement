...
const defaults = reactive({
    ...
    skipAdjustYflipForAuto: options?.skipAdjustYflipForAuto ?? false, // adjustY value from 12 to 12 and from -12 to 12
...
const fn = {     
    ...
    onClickPrev: function () { 
        ...
        switch (current_view.value) {
            case 'days':
                date = moment(picker.date).subtract(defaults.calendarQuantity, 'months').format(FORMATS.date);  
                picker.date = date              
                break;        
            case 'years':
    ...
    onClickNext: function () { 
        ...
        switch (current_view.value) {
            case 'days': 
                date = moment(picker.date).add(defaults.calendarQuantity, 'months').format(FORMATS.date);  
                picker.date = date 
...
function daysOfMonth(year, monthIndex, FORMATS, { currentMonth } = {}) {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    const currentDay = moment(firstDayOfMonth).date(day); // clone to avoid mutation
    const _day = {
      ...
      month_index: currentDay.month(), // 0-based month index via moment
...
function getMonthCalendarDays(DATE = null) {
  const baseDate = moment(DATE || picker.date);
  const monthIndex = baseDate.month();
  const year = baseDate.year();

  const days = daysOfMonth(year, monthIndex, FORMATS, { currentMonth: true });
  ...
  // Left tailing
  const prevMonth = moment(baseDate).subtract(1, 'months');
  const previous_month_days = daysOfMonth(prevMonth.year(), prevMonth.month(), FORMATS).slice(-startFrom);

  const days_after_left_tailing = startFrom ? [...previous_month_days, ...days] : [...days];

  // Right tailing
  const nextMonth = moment(baseDate).add(1, 'months');
  const next_month_days = daysOfMonth(nextMonth.year(), nextMonth.month(), FORMATS);
