...
function getFixedParentOffset(element) {
    ...
    while (parent) {
        ...
        if (style.position === "fixed") {
            let bound = parent.getBoundingClientRect(); // Return the fixed parent's rect
    ...
    return position; // No fixed parent found
}
...
function setDivStyles(e=null){
    ...
    timeout = setTimeout(() => {
        ...
        if(input && wrapper){            
            ...
            // let parentFixedElementBound = getFixedParentOffset(input)
            // console.log({parentFixedElementBound, inputBound, input})
            ...
            if(pickerArea){
                ...
                if(bodyBound.width > 450){
                     ...
                     /**
                     * Try graving the position
                     */
                    if(displayIn === 'auto'){
                        let {innerWidth, innerHeight} = window;
                        let has_available_space_in_bottom = (innerHeight - inputBound.bottom) >= pickerBound.height;
                        let has_available_space_in_top = inputBound.top >= pickerBound.height;
                        let leftSpace = inputBound.left;
                        let righSpace = innerWidth - inputBound.right;
                        // let has_available_space_in_left = 

                        if(has_available_space_in_bottom){
                            if(leftSpace >= (righSpace - 50)){
                                displayIn = 'bottom_left';
                            } else {
                                displayIn = 'bottom_right';
                            }
                        }
                        else if(has_available_space_in_top){
                            if(leftSpace >= (righSpace - 50)){
                                displayIn = 'top_left';
                            } else {
                                displayIn = 'top_right';
                            ...
                            if(!clonedOptions?.skipAdjustYflipForAuto){
                                adjustY = -(adjustY)     
                            }
                        }
                        else {
...
<template>
    <template v-if="modalValueIntialized">
        ...
        <template v-if="get_position !== 'modal'">
            <template v-if="get_position.startsWith('inline')"> 
        ...
        @click.stop.prevent="onClickShow"
