...
console.log('props.modelValue?.totalPages', props.modelValue);
                    ...
                    <template v-for="(page_no, index) in rangeArray" :key="index">
                        <slot name="pageNumbers">
                            <template v-if="page_no <= totalPage">
                                <li class="page-item cp" :class="{'active': page_no == helper.getQuery('page')}" @click.prevent="goToPage(page_no)"><a class="page-link cp" :href="pageKey ? getLink(page_no) : false"> {{ page_no }} </a></li>
                            </template>
                        </slot>
                    </template>                        
                </template>                        
...
.page-item.active .page-link{
    ...
    box-shadow: 0px 2px 3px 1px rgba(172, 255, 47, 0.281);
    text-shadow: 0px 0px 2px greenyellow;
}
.page-link:focus {
    box-shadow: 0 0 0 0.25rem #21851457;;
}
</style>